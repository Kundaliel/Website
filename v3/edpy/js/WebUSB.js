...
function webUSBTransferInReady(result) {
    ...
    if (result.status == "ok") {
        console.log("webUSBTransferInReady", result.data.byteLength);
        if (result.data.byteLength == 6 &&
            ...
            console.log("usb data not handled by this function");
            if (webUSBPendingInResolve !== null) {
                console.log("usb data pending");
                webUSBPendingInResolve(result);
                webUSBPendingInResolve = null;
            } else {
                console.log("usb data saved for next");
...
async function webUSBOpenConnection() {
    ...
    webUSBDevice = await navigator.usb.requestDevice({
        filters: [{
            vendorId: RELESE_VENDOR_ID,
            productId: RELESE_PRODUCT_ID
        }]
    });
...
async function webUSBTransferIn(len) {
    let prom;
    console.log("start of transfer in");
    ...
    console.log("Awaiting promise return");
...
async function webUSBCommandPutAppFirmware(firmware, checkValidReturn) {
    ...
    await sleep(0.3);
    let result = await webUSBTransferIn(8);
    console.log("back from in");
    if (result.status == "ok" &&
        result.data.getUint8(0) == EV3_WEBUSB_HEADER &&
        result.data.getUint8(1) == EV3_WEBUSB_CMD_PUT_APP_FIRMWARE) {
        // can start programming
        console.log("can start programming");
    } else {
        alert("download failed");
        return false;
    }
...
async function SPK_webUSBCommandUserDataFrom(n) {
...
async function OFF_getPersistentData() {

    const data = [0, await webUSBCommandGetPersistentData()];
    const data_version = data[1][0] & 0xf;
    console.log(data);
    var stats = []
    //writeToUSBDataBox("PERSISTENT DATA")
    //writeToUSBDataBox("data_length: " + data[1].length)

    stats.push("0x" + getU32(data).toString(16)); // "data_magic:
    console.log("data_checksum: 0x" + getU32(data).toString(16));
    if (data_version >= 3) {
        stats.push(getU8(data)); //"irc48m_trim: " +
        skipBytes(data, 3);
    }

    stats.push(getU16Array(data, 10)); //"pwm_left_forward: "
    stats.push(getU16Array(data, 10)); //"pwm_left_backward: "
    stats.push(getU16Array(data, 10)); //"pwm_right_forward: "
    stats.push(getU16Array(data, 10)); //"pwm_right_backward: "

    stats.push(getU16Array(data, 2)); //"ir_obstacle_freq: "

    for (let i = 0; i < 8; ++i) {
        stats.push(getU32Array(data, 2)); //"ir_remote_learn[" + i + "]: "
    }

    stats.push(getU32(data)); //"turn_on_count: "
    stats.push(getU32(data)); //"usb_connect_count: "
    stats.push(getU32(data)); //"program_user_data_count: "
    stats.push(getU16(data)); //"barcode_read_success_count: "
    stats.push(getU16(data)); //"barcode_read_failure_count: "
    stats.push(getU16Array(data, 4)); //"screen_program_success_count: "
    stats.push(getU16Array(data, 4)); //"screen_program_failure_count: "
    stats.push(getU16(data)); //"button_triangle_press_count: "
    stats.push(getU16(data)); //"button_square_press_count: "
    stats.push(getU16(data)); //"button_round_press_count: "
    stats.push(getU16(data)); //"motor_calibration_success_count: "
    stats.push(getU8Array(data, 20)); //"motor_calibration_failure_count_left: "
    stats.push(getU8Array(data, 20)); //"motor_calibration_failure_count_right: "
    stats.push(getU32(data)); //"motor_tick_count_left_forward: "
    stats.push(getU32(data)); //"motor_tick_count_left_backward: "
    stats.push(getU32(data)); //"motor_tick_count_right_forward: "
    stats.push(getU32(data)); //"motor_tick_count_right_backward: "
    stats.push(getU32(data)); //"temperature_above_67c_with_usb_count: "
    stats.push(getU32(data)); //"temperature_above_67c_seconds: "
    stats.push(getU32(data)); //"battery_empty_count: "
    stats.push(getU32(data)); //"battery_full_count: "
    stats.push(getU32Array(data, 6)); //"battery_charging_seconds: "
    stats.push(getU32(data)); //"battery_charged_seconds: "
    stats.push(getU32(data)); //"battery_use_seconds: "
    stats.push(getU32(data)); //"battery_over_charged_seconds: "
...
async function getPersistentData() {
    ...
    stats.push({
        label: "data_magic",
        value: "0x" + getU32(data).toString(16)
    });
    stats.push({
        label: "data_checksum",
        value: "0x" + getU32(data).toString(16)
    ...
    if (data_version >= 3) {
        ...
        stats.push({
            label: "width_trim_ccw",
            value: getS8(data)
        });
    }

    stats.push({
        label: "pwm_left_forward",
        value: getU16Array(data, 10)
    });
    stats.push({
        label: "pwm_left_backward",
        value: getU16Array(data, 10)
    });
    stats.push({
        label: "pwm_right_forward",
        value: getU16Array(data, 10)
    });
    stats.push({
        label: "pwm_right_backward",
        value: getU16Array(data, 10)
    });
    stats.push({
        label: "ir_obstacle_freq",
        value: getU16Array(data, 2)
    });

    for (let i = 0; i < 8; ++i) {
        stats.push({
            label: `ir_remote_learn[${i}]`,
            value: getU32Array(data, 2)
        });
    }
    ...
    stats.push({
        label: "usb_connect_count",
        value: getU32(data)
    });
    stats.push({
        label: "program_user_data_count",
        value: getU32(data)
    });
    stats.push({
        label: "barcode_read_success_count",
        value: getU16(data)
    });
    stats.push({
        label: "barcode_read_failure_count",
        value: getU16(data)
    });
    stats.push({
        label: "screen_program_success_count",
        value: getU16Array(data, 4)
    });
    stats.push({
        label: "screen_program_failure_count",
        value: getU16Array(data, 4)
    });
    stats.push({
        label: "button_triangle_press_count",
        value: getU16(data)
    });
    stats.push({
        label: "button_square_press_count",
        value: getU16(data)
    });
    stats.push({
        label: "button_round_press_count",
        value: getU16(data)
    });
    stats.push({
        label: "motor_calibration_success_count",
        value: getU16(data)
    });
    stats.push({
        label: "motor_calibration_failure_count_left",
        value: getU8Array(data, 20)
    });
    stats.push({
        label: "motor_calibration_failure_count_right",
        value: getU8Array(data, 20)
    });
    stats.push({
        label: "motor_tick_count_left_forward",
        value: getU32(data)
    });
    stats.push({
        label: "motor_tick_count_left_backward",
        value: getU32(data)
    });
    stats.push({
        label: "motor_tick_count_right_forward",
        value: getU32(data)
    });
    stats.push({
        label: "motor_tick_count_right_backward",
        value: getU32(data)
    });
    stats.push({
        label: "temperature_above_67c_with_usb_count",
        value: getU32(data)
    });
    stats.push({
        label: "temperature_above_67c_seconds",
        value: getU32(data)
    });
    stats.push({
        label: "battery_empty_count",
        value: getU32(data)
    });
    stats.push({
        label: "battery_full_count",
        value: getU32(data)
    });
    stats.push({
        label: "battery_charging_seconds",
        value: getU32Array(data, 6)
    });
    stats.push({
        label: "battery_charged_seconds",
        value: getU32(data)
    });
    stats.push({
        label: "battery_use_seconds",
        value: getU32(data)
    });
    stats.push({
        label: "battery_over_charged_seconds",
        value: getU32(data)
    });
